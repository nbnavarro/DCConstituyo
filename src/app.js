...
app.on('error', async (err, ctx) => {
  ...
  console.table({ 'status error': ctx.status });
  console.table({ 'Razon del error:': 'abajo' });
  console.log(err);
  /* eslint-enable no-alert, no-console */
  if (ctx.status === 500) {
    await ctx.redirect(ctx.router.url('errors.500'));
  } else if (ctx.status === 404) {
    await ctx.redirect(ctx.router.url('errors.404'));
  } else if (ctx.status === 403) {
    await ctx.redirect(ctx.router.url('errors.403'));
  // } else if (ctx.status === 401) {
  //   await ctx.redirect(ctx.router.url('errors.401'));
  }
});
